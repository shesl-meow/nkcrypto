...
void Md5Hash::init() {
    this->digest = BitsetMethod::convertFrom<128>(INIT_DIGEST, 8);
}

/*
 * decodes input (unsigned char) into output (uint32_t). Assumes len is a multiple of 4.
 */
inline void decode(uint32_t output[], const uint8_t input[], size_t len)
{
    assert(len % 4 == 0);
    for (size_t i = 0, j = 0; j < len; i++, j += 4) {
        output[i] = (uint32_t)input[j];
        output[i] |= ((uint32_t)input[j+1]) << 8u;
        output[i] |= ((uint32_t)input[j+2]) << 16u;
        output[i] |= ((uint32_t)input[j+3]) << 24u;
    }
}
 ...
 */
inline void encode(uint8_t output[], const uint32_t input[], size_t len)
{
    for (size_t i = 0, j = 0; j < len; i++, j += 4) {
...
void Md5Hash::transform(const uint8_t block[64])
{
    ...
    static const bitset512 mask512 = (1ul << 32u) - 1;
    uint32_t A = (digest >> (32u * 3) & mask128).to_ullong(), a = A;
    uint32_t B = (digest >> (32u * 2) & mask128).to_ullong(), b = B;
    ...
    digest = bitset128(A + a) << (32u * 0);
    digest |= bitset128(B + b) << (32u * 1);
    digest |= bitset128(C + c) << (32u * 2);
    digest |= bitset128(D + d) << (32u * 3);
...
void Md5Hash::update(const unsigned char input[], size_t length)
{
    // compute number of bytes mod 64
    size_t index = count[0] / 8 % BLOCK_SIZE;
    ...
    if (length >= firstpart)
    {
        ...
        pblock = BitsetMethod::convertFrom<512>(buffer, 8);
        transform(buffer);
...
std::string Md5Hash::hexdigest() const
{
    ...
    static const bitset128 mask = 0xfu;
    for (size_t i = 0; i < 128; i += 4) {
        sstream << std::hex << ((digest >> i) & mask).to_ullong();
    }
    return sstream.str();
}
